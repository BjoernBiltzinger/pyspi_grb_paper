...
\abstract {
  In the last years it has been shown, that it is needed to fit physical models to Gamma-Ray Burst (GRB) data to get more insights into their emission mechanism. For the energy range between $\approx$ 10 keV to $\approx$ 10 MeV Fermi/GBM has been the main instrument to analyze GRBs. In this paper we show how we can add INTEGRAL/SPI to this analysis. INTEGRAL/SPI covers a similar energy range like Fermi/GBM and has a very good energy resolution of 2.2 keV at 1.3 MeV. Therefore INTEGRAL/SPI is an ideal instrument to precisely measure the curvature of the spectrum. We present PySPI, a new analysis software we developed to analyze GRB data from SPI, and apply it to the GRB 120711A. We show that PySPI improves the analysis of SPI GRB data compared to the official analysis software. Also we demonstrate that the GBM and the SPI data of this GRB can be fitted well with a physical synchrotron model and that combining both, reduces the allowed parameter space. This evinces that SPI can play an important role in GRB model fitting.
}
...
\subsection{PySPI}
\label{pyspi}
To fit the SPI data of GRBs, we developed a new python package PySPI \citep{joss}, which is an open source analysis tool. In the following we will summarize the main points of GRB analysis within PySPI.

\subsubsection*{Background}

GRBs are transient sources with a typical duration only up to a few tens of seconds. Therefore we can use the time intervals during a science window, when the transient source is not active, as an independent temporal off-source observation (see Fig. \ref{fig:lightcurve}). Similar to other instruments, that use spatially off-source observation as an independent background measurement. From the background observations we construct the probability of the background model rates per energy channel from a poisson distribution, like given in Eq. \ref{eq:poisson_bkg}, where $b_{i}$ are the background rates per energy channel, $B_{i}$ are the detected counts in the off-source observation and $t_{b}$ is the exposure of the off-source observation.
\begin{equation}
...
With the defined background treatment, we can construct a Poisson likelihood with a Poisson measurement of background, like given in Eq. \ref{eq:likelihood_m_and_b}. Here $\theta$ summarizes all the parameters of the model, $D_{i}$ are the measured counts in the selected active time interval of the transient source, $m_{i}(\theta)$ are the predicted counts from the model evaluated at the parameters $\theta$ and $t_{d}$ is the exposure of the selected active time interval.
...
Now we can marginalize over the parameters $b_{i}$, that we are not interested in and that we have no further information about a priori, despite the fact that they can not be negative, which leaves us with Eq. \ref{eq:likelihood_marg}.
...
These values can then be substituted in the likelihood Eq. \ref{eq:likelihood_m_and_b} to eliminate the $b_{i}$ dependency. This approximation gives the exact solution for the maximum of the likelihood, for likelihood values close to the maximum, the assumption is that most of the likelihood in the integrand in Eq. \ref{eq:likelihood_marg} is in a small area around $b_{i,max}$. This assumption is used in many spectral analyses and works very well as long as one does not have too few counts in the background observation \footnote{For a derivation see https://giacomov.github.io/Bias-in-profile-poisson-likelihood/}.
...
PySPI is an open source software, which is publicly available on GitHub \citep{joss}. There is also a documentation, that shows how to use it. \footnote{For more information about PySPI see pyspi.readthedocs.io/}
...
\end{equation}
\noindent
Here $y^{\textrm{obs}}$ are the observed data, $\textrm{P}(\theta|y^{\textrm{obs}})$ is the posterior distribution of the model parameters $\theta$ given the observed data, $\textrm{P}(y^{\textrm{sim}}|\theta)$ is the probability of new data given the parameters of the model and $y^{\textrm{sim}}$ are the simulated data.
...
Next, we analyze the SPI data with PySPI and the GBM data with the GRB analysis within 3ML for a band function model. Fig. \ref{fig:corner_gbm_pyspi_band} displays the corner plots for these fits, demonstrating that the results for the spectral shape from SPI and GBM are in agreement. It also shows that the calibration for SPI and GBM are well aligned within only $\approx$ 10\% difference in this case.
