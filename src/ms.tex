...
Next, we analyze the SPI data with PySPI and the GBM data with the GRB analysis within 3ML (cite a paper for this?) for a band function model. Fig. \ref{fig:corner_gbm_pyspi_band} displays the corner plots for these fits, showing that the results for the spectral shape form PySPI and GBM are in agreement. It also shows that the calibration for SPI and GBM are well aligned within only $\approx$ 10\% difference.
...
The final analysis is a joint fit for the SPI and the GBM data with the physical synchrotron model (see \ref{synch}).
